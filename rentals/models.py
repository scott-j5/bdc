...
class RentalProductManager(models.Manager):
	...
	def available(self, dt_range):
		...
		).values_list('rental_product__id', flat=True)
		return qs.exclude(id__in=unavailable_products)

	# Bind rental prices to a qs of rental products for a set date range
	# dt_range: [start_date, end_date]
...
class RentalProduct(Product):
	...
	def unavailable_dates(self):
		unavailable = Rental.objects.filter(rental_product=self).values_list('rental_start', 'rental_end_inc_turnaround')
		return unavailable
...
class RentalFulfilment(ProductFulfilment):
	...
	@property
	def price(self):
		return self._price if self._price else self._calculate_price()

	# Calculates the total price of a rental based on all applicable price adjustments
	def _calculate_price(self):
		rental_product_price = self.rental_product.price
		if CHARGE_RENTAL_DAILY:
			total_price = rental_product_price * self.duration_days
		else:
			total_price = rental_product_price * self.duration_hours
		adjustment_val = 0
		
		for adj in self.active_price_adjustments:
			if CHARGE_RENTAL_DAILY:
				applicable_range = datediff_days(get_date_overlap([adj.period_start, adj.period_end], [self.rental_start, self.rental_end]))
			else:
